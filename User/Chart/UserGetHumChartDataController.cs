...
{
    public class UserGetHumChartDataController : ApiController
    {
        public static string strConn = ConfigurationManager.ConnectionStrings["DefaultDataSource"].ConnectionString;

        // GET: api/UserGetHumChartData
        public IEnumerable<string> Get()
        {
            return new string[] { "value1", "value2" };
        }

        // GET: api/UserGetHumChartData/5
        public string Get(int id)
        {
            return "value";
        }

        // POST: api/UserGetHumChartData
        public void Post([FromBody]string value)
        {
        }

        // PUT: api/UserGetHumChartData/5
        public void Put(int id, [FromBody]string value)
        {
        }

        // DELETE: api/UserGetHumChartData/5
        ...
        public IHttpActionResult GetSensorData(string UserMail, string StartTime, string EndTime)
        {
            ...
            scChart.CommandText = @"
                                    select convert(varchar(16),Time,120) Time,Humidity from SensorTemHumData Where Time BETWEEN @StartTime AND @EndTime AND DATEPART(MINUTE, Time )=0 
